...
- **A new customiser shape must not push `RAYON_MAX` past 1.15.** The export frame
  ...
  and the shape weighs 0.899 in equivalent radius, the band where the cloud and the
  pebble already live. A user's shape only replaces the body on `baseBody` states
